...
checker_T = Callable[[Profile], Tuple[bool, Optional[str]]]


class CheckAttacher:
    def __init__(self) -> None:
        self.checks: List[checker_T] = []

    def attach(self, check_function: checker_T):
...
class ExperiencedPythonDeveloperFilter(ProfileFilter):
    ...
    @checker.attach
    def check_latest_2_faang(
        self, profile: Profile
    ) -> Tuple[bool, Optional[str]]:
        latest_companies = [
            experience.company_name for experience in profile.experiences[:2]
        ]
        if not any(
            map(
                lambda company_name:
                    company_name.strip().lower() in self.faang_companies,
                latest_companies
            )
        ):
    ...
    @checker.attach
    def check_latest_3_swe_or_backend(
        self, profile: Profile
    ) -> Tuple[bool, Optional[str]]:
        latest_job_titles = [
            experience.job_title for experience in profile.experiences[:3]
        ]
        if not all(
            map(
                lambda job_title:
                    job_title.strip().lower() in self.target_job_titles,
                latest_job_titles
            )
        ):
            return False, f"{', '.join(latest_job_titles)} are not all " \
                "Backend developer or Software engineer roles"
        return True, None

    @checker.attach
    def check_experience_over_8_years(
        self, profile: Profile
    ) -> Tuple[bool, Optional[str]]:
        ...
        if full_years < 8:
            return False, f"Experience ({full_years} years) " \
                "is less then 8 years"

        return True, None

    @checker.attach
    def check_latest_job_python_and_cpp(
        self, profile: Profile
    ) -> Tuple[bool, Optional[str]]:
        ...
        if "c++" not in latest_job_skills or "python" not in latest_job_skills:
            return False, "Python and C++ are not " \
                f"in the latest company's skills: {latest_job_skills}"

        return True, None

    @checker.attach
    def check_london_location(
        self, profile: Profile
    ) -> Tuple[bool, Optional[str]]:
